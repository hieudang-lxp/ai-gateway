...
export function usageNetwork(rows: ModelUsage[], metric: Metric, modelLimit = 5, locale = i18n.resolvedLanguage): UsageNetwork {
  const t = i18n.getFixedT(locale ?? 'en', 'sessions');
  const nodes: NetworkNode[] = [{ id: 'total', label: t('allUsage'), kind: 'total', source: '', color: '#e9f2ff', usage: sumUsage(rows), position: [-6.7, 0, 0] }];
  ...
  sources.forEach((source, index) => {
    ...
    nodes.push({ id, label: source === 'claude_gateway' ? t('claudeGateway') : sourceName(source), kind: 'source', source, color, usage: sumUsage(sourceRows), position: [-2.3,y,0] });
    edges.push({ from: 'total', to: id });
    const leaves = sourceRows.slice(0,modelLimit).map(row => ({ usage: row, label: row.model || t('modelNotRecorded'), models: undefined as string[] | undefined }));
    if (sourceRows.length > modelLimit) leaves.push({ usage: sumUsage(sourceRows.slice(modelLimit)), label: t('otherModels', { count: sourceRows.length-modelLimit, amount: formatNumber(sourceRows.length-modelLimit, undefined, locale) }), models: sourceRows.slice(modelLimit).map(row=>row.model || t('modelNotRecorded')) });
