  ...
  return (
    <div className={`flex min-w-0 flex-col gap-2 ${alignEnd ? "md:items-end" : "items-start"}`}>
      <span className="break-words font-semibold tabular-nums text-sky-950">
        {session.unknown_cost_calls === session.calls && session.calls > 0 ? "Unavailable" : fmt(session.known_cost_usd)}
      </span>
      <div className={`flex min-h-6 flex-wrap items-center gap-2 ${alignEnd ? "md:justify-end" : ""}`}>
        {session.estimated_cost_calls > 0 && <Badge variant="secondary">Estimate</Badge>}
        {session.fallback_cost_calls > 0 && <Badge variant="outline">Sol fallback</Badge>}
        {session.unknown_cost_calls > 0 && <span className="text-xs text-slate-500">{count(session.unknown_cost_calls)} unpriced</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <article className="grid min-w-0 gap-4 border-b border-slate-100 py-5 last:border-0 md:grid-cols-[minmax(0,1fr)_18rem] md:gap-6">
      <div className="min-w-0">
        ...
        {!session.title.trim() && <p className="mt-1 text-xs text-slate-500">No title recorded · <span className="font-mono" title={session.session_id}>ID {shortSessionID(session.session_id)}</span></p>}
        <p className="mt-1 break-all text-xs text-slate-500">{session.project || "Workspace unavailable"}</p>
        <p className="mt-1 break-words text-xs text-slate-500">{session.models?.join(" · ") || "Model unavailable"}</p>
      </div>
      <div className="min-w-0 space-y-2 md:text-right">
        <SessionValue session={session} alignEnd />
        <p className="text-xs tabular-nums text-slate-500">{count(tokens(session))} tokens · {count(session.calls)} {session.calls === 1 ? "event" : "events"}</p>
      </div>
    </article>
  );
}
