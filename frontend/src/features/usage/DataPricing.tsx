  ...
  return (
    <div className="grid grid-cols-1 gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Button variant="outline" disabled={isFetching} onClick={() => void refetch()}>{isFetching ? "Refreshing…" : "Refresh status"}</Button>
      </div>

      {isPending && <Card role="status" className={card}>Checking your sources…</Card>}
      {error && <Alert className="bg-amber-50 text-amber-900"><AlertDescription>{error.message}{data && " Amounts below are from the last available snapshot."}</AlertDescription></Alert>}
      {data && <>
        <Alert role="status" aria-label="Sync summary" className={`flex flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-6 ${ready === 3 ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}>
          <div>
            <AlertTitle className={`font-semibold ${ready === 3 ? "text-emerald-900" : "text-amber-900"}`}>{error ? "Cannot confirm sync status" : ready === 3 ? "All 3 tools are synced" : `${ready} of 3 tools are synced`}</AlertTitle>
            <AlertDescription className={`mt-1 text-xs ${ready === 3 ? "text-emerald-800" : "text-amber-800"}`}>{ready === 3 ? "Automatic collection is running. The latest available records have been imported." : "Check the source marked below. Saved usage is still available."}</AlertDescription>
          </div>
        ...
        <section aria-labelledby="amounts-heading">
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {collectorIds.map(id => {
              ...
              return <Card key={id} className={card}>
                <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold text-sky-950">{sourceNames[id]}</h3><Badge variant="secondary" className={healthy ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}>{healthy ? "Synced" : error ? "Unverified" : "Check connection"}</Badge></div>
        ...
        <Card aria-labelledby="accuracy-heading" className={card}>
        ...
        <Card className={card}><Accordion type="single" collapsible><AccordionItem value="technical" className="border-0">
          <AccordionTrigger className="text-sm font-semibold text-sky-800">Technical details & collection limits</AccordionTrigger>
          <AccordionContent>
