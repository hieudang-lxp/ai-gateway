...
import { pages, usePage } from "./lib/navigation";
...
export default function App() {
  ...
  useEffect(() => {
    document.title = `${pages.find(item => item.id === page)?.label} · AI Gateway`;
  }, [page]);
  return (
    <CurrencyProvider>
      <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-white text-base text-slate-800">
        ...
        <main id="main-content" tabIndex={-1} className="mx-auto max-w-6xl px-4 py-7 focus:outline-none sm:px-6 sm:py-9">
          {page === "sessions" ? <Sessions period={period} setPeriod={setPeriod} /> : page === "insights" ? <Insights period={period} setPeriod={setPeriod} /> : page === "data-pricing" ? <DataPricing period={period} /> : <>
